...
A simple tool to keep a local repository synchronized with a remote one. It supports manual synchronization via HTTP endpoints and automatic synchronization based on a cron schedule.

## Features

- **Automated Sync**: Synchronize the repository at regular intervals using cron expressions.
- **Manual Sync**: Trigger an immediate synchronization via a HTTP POST request.
- **Status Monitoring**: Check the last sync time and history via a HTTP GET request.
...
- **Containerized**: Can be deployed as a container.
...
- `500 Internal Server Error`: An error occurred, including synchronization errors.
